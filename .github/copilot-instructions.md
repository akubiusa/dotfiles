...
- `update.sh` を変更する場合、AI CLI の throttle、同時実行ロック、`chezmoi update` 後の managed global config に対する `mise install` が維持されているか確認する。systemd の `chezmoi-update.service` / `.timer` は dotfiles の管理対象に戻さない。
