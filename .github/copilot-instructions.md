...
- `update.sh` または systemd updater unit を変更する場合、AI CLI の throttle、同時実行ロック、timer の `Persistent=true`、非対話更新前の drift target backup、managed global config に対する `mise install` を維持し、`tests/unit/test_update.sh` と `tests/integration/test_chezmoi_apply.sh` で確認する。backup 失敗時は apply せず、systemd unit は `home/dot_config/systemd/user/` の chezmoi source で管理する。
