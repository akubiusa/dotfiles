...
if [[ "$PR_ARG" =~ ^[0-9]+$ ]]; then
  PR_NUMBER="$PR_ARG"
  if ! resolve_repo_from_origin; then
...
JQ_FILTER='[.data.repository.pullRequest.reviews.nodes[] | select(.author.__typename == "Bot" and (.author.login | ascii_downcase | contains("copilot")) and (.state == "COMMENTED" or .state == "APPROVED") and .submittedAt != null)] | length'
...
notify_discord() {
  ...
  if [[ -f "$HOME/.env" ]]; then
    ...
    source "$HOME/.env"
    if [[ -n "${DISCORD_CODEX_MENTION_USER_ID:-}" ]]; then
      mention_prefix="<@${DISCORD_CODEX_MENTION_USER_ID}> "
    fi
  fi

  local payload
  payload=$(jq -n \
    --arg content "${mention_prefix}Codex CLI Notification" \
    --arg title "$title" \
    --arg desc "$desc" \
