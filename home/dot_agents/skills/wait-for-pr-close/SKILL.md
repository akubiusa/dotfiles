...
`$wait-for-pr-close <PR 番号または URL>` は `$pr-health-monitor` の foreground watcher 部分を案内する。watcher は PR state、checks、conflict、actor を問わない review feedback を poll して XDG state に event を記録するだけで、`$pr-cleanup`、Git 操作、GlitchTip callback は実行しない。
...
3. merge/close、CI failure、conflict、review feedback は pending event として `${XDG_STATE_HOME:-~/.local/state}/codex-pr-monitor/` に保存される。review feedback は `$handle-pr-reviews` へ送られる。連続 5 回の API failure で watcher は停止する。
