...
- Work not tied to a GitHub Issue (Jira-linked `ticket-pr` work, standalone investigations, pre-Issue discussions) — invoke the `opengist` skill instead.
...
- Verify no sensitive information (tokens, passwords, internal URLs, credentials) is included before posting — same check required before any opengist/Jira post (see `rules/security.md`).
